...
;;
;;
;; Copy what can from ART3w2eqs.lisp (just renamed version of ART2 -- it initially worked fine with these other changed files!!
;;
...
;;1. SSSS PROBLEM--All F3 x values are very, very small after reset due to being set small by competition before.  Doesn't leave any values--except tiny ones.  Compet values must be higher than reset ones or may choose same node again.
;;
;;
;;2. I SET TO RESET MAX YI-2-2.  NOT SURE THIS MAKES SENSE--TEST AND THINK OF ALTERNATIVES
...
CONSTANTS-PARAMETERS:~% *delta-t= ~A; *n-cycles-per-tdelta= ~A *n-pattern-cycles= ~A ~%  *p1-all= ~A; *p2-all= ~A; *p3-all= ~A; *p4-3= ~A; *p5-all= ~A; *P6-all= ~A~%*def-sigmoid-val= ~A;  ORIG-ART2:  a= ~A; b= ~A; c= ~A; d= ~A; e= ~A; theta= ~A; *vigilance= ~A *vigilance-multiplier= ~A ~%*p1-all= ~A  *onCenterThreshold= ~A; *min-input-criteria= ~A; ~%*min-g= ~A;*reset-y-criteria= ~A; ~%INITIAL VALUES:~% *art-initial-x= ~A; *art-initial-y= ~A; Weights: upLR= ~A downLR= ~A; *wUpInitLo= ~A, *wUpInitHi= ~A~%*wDnInitLo= ~A *wDnInitHi= ~A~%
*def-nonreset-val= ~A; *def-reset-xy-val= ~A; *def-comp-val= ~A~%" date-time *delta-t *n-cycles-per-tdelta *n-pattern-cycles *p1-all *p2-all *p3-all *p4-3 *p5-all *P6-all  *def-sigmoid-val  a b c d e theta *vigilance *vigilance-multiplier  *p1-all  *onCenterThreshold *min-input-criteria  *min-g *reset-y-criteria *art-initial-x *art-initial-y  upLR downLR *wUpInitLo *wUpInitHi *wDnInitLo *wDnInitHi *def-nonreset-val  *def-reset-xy-val *def-comp-val ))))


    (setf *art-data-text (append *art-data-text (list (format nil ">> ART3 DESIGN PARAMETERS:~%COMPETITION PARAMETERS:  *F3-all-or-none-competative-output-p= ~A;  *F1-all-or-none-competative-output-p= ~A;   *F3-compet-every-nth-time-interval= ~A;   *test-compet-overall-cycle-nums-list= ~A;   *F3L1-compet-p  ~%RESET PARAMETERS:= ~A;   *recalc-F3L1xy-postreset-p= ~A;   *recalc-yi-3-3-postcompet-p= ~A;   *F2-test-reset-p= ~A;   *F3-test-reset-p= ~A;   *test-reset-every-nth-pattern-cycle= ~A;   *test-reset-every-nth-time-interval= ~A;   *test-reset-overall-cycle-nums-list= ~A;   *reset-all-F1F2-p= ~A;   *reset-all-F1F2L1-p= ~A;   *reset-all-F1F2L2-p= ~A;   *reset-all-F3Layer-maxs-p= ~A;   *find-F3L1y-max-output-p= ~A;   *find-F3L3y-max-output-p= ~A;   *reset-F2-p= ~A;   *reset-XI-1-3-p= ~A;   *reset-XI-2-3-p= ~A;   *reset-XI-3-3-p= ~A   ~%" *F3-all-or-none-competative-output-p *F1-all-or-none-competative-output-p *F3-compet-every-nth-time-interval *test-compet-overall-cycle-nums-list *F3L1-compet-p *recalc-F3L1xy-postreset-p *recalc-yi-3-3-postcompet-p *F2-test-reset-p *F3-test-reset-p *test-reset-every-nth-pattern-cycle *test-reset-every-nth-time-interval *test-reset-overall-cycle-nums-list *reset-all-F1F2-p *reset-all-F1F2L1-p *reset-all-F1F2L2-p *reset-all-F3Layer-maxs-p *find-F3L1y-max-output-p *find-F3L3y-max-output-p *reset-F2-p *reset-XI-1-3-p *reset-XI-2-3-p *reset-XI-3-3-p   ))))
...
;; SO DO ONLY AFTER MULTIPLE CYCLES 
;;Do reset before f3-cycle  to prevent make-graph-points-list from having no f3 calc data
  ...
  ;;COMPETE ONLY IF "CHOICE" ALL-OR-NONE F3 MODEL
...
(defun F2-cycle ()  
  ...
  (loop
   ...
   ;;end loop
     ...
     (setf *art-data-text (append *art-data-text (list (format nil  "~%Step 1: FIND xi-1-2-vals= ~A~%FOR each i;  xi-1-2=(* *p2-all  (+ yi-3-1  (* a  sigp-yI-2-2)));  *p2-all= ~A yi-3-1-vals  a=~A sigp-yI-2-2= ~A  yi-2-2-vals "   xi-1-2-vals  *p2-all   a sigp-yI-2-2  ))))
  ...
  ;; STEP 2: UPDATE YI-1-2 USING XI-1-2, L2NORM-XI-1-2
    ...
    (when  *reset-F2-p 
      (multiple-value-bind (max-yi-2-2-index  max-yi-2-2-val)
        ...
        (when (> (getsymval 'resetnin (list i 2 2)) *def-nonreset-val)                  
   ...
   (loop
     ...
     (when
      (or *reset-XI-1-3-p *reset-all-f3layer-maxs-p)
       ...
       (when
        (> resetnout *def-nonreset-val) 
         (setf xi-1-3 *def-reset-xy-val
      ...
      (when (or (null resetnout) (null xi-1-3))
       (setf  yI-2-3 (getsymval 'y (list j 2 3))
              sigp-yI-2-3  (*  *p3-all  (sigmoid  yi-2-3))
     ...
     ;;(if (> *print-details  1) (afout 'out (format nil  "In F2STMcycle, Step 2: FIND yi-3-3 FOR j= ~A,  yi-3-3=~a ; reset-noutput= ~A"  j  yi-3-3 reset-noutput )))
     ;;was (if (> *print-details  1) (afout 'out (format nil  "In F2STMcycle, Step 2: FIND y-output FOR j= ~A,  y-output=~a ; reset= ~A"  j  y-output reset )))
   ...
   ;;Step 1.2:  Adjust INPUTS xi-2-3 for RESET ;; do it only in RESET--not work?
    ...
    #| older
     (when *reset-XI-2-3-p
       ...
       (cond
        ((and resetnout (> resetnout *def-nonreset-val))
         (setf xi-2-3 *def-reset-xy-val))
        (t
         (setf yi-1-3  (getsymval 'y (list j 1 2))
               sig-yi-1-3 (sigmoid yi-1-3)
               yi-3-3 (getsymval 'y (list j 3 3))  
               sigp-yI-3-3 (* *p3-all (sigmoid  yi-3-3 ))
               xi-2-3 (+ sig-yi-1-3 sigp-yI-3-3)))) ;;*p2-all= 10?
...
(loop
   ...
   #|older (loop
     ...
     (loop
      ...
      ;;(if (= *print-details 3) (afout 'out (format nil  "~%Step 3: FIND yI-2-3, FOR J= ~A, ~& yI-2-3= (/ xI-2-3init  L2norm-xI-2-3)= ~A;  xI-2-3init= ~A  L2norm-xI-2-3= ~A" i  yI-2-3  xI-2-3init  L2norm-xI-2-3)))
    ...
    (multiple-value-bind (y-max-index y-max-value)
      ...
      (multiple-value-bind (dims-list rootstr )
       ...
       (when add-data-text-p
         (setf *art-data-text (append *art-data-text (list (format nil "~%===> IN COMPETITIVE-F-OUTPUT; ~%FOR n-nodes= ~A; y-max-index= ~A y-max-value ~A;   *onCenterThreshold= ~A~%POST-COMPET ~A-vals= ~A~%" n-nodes y-max-index y-max-value   *onCenterThreshold outvar-sym outvar-vals)))))
...
;;
;;ddd
(defun test-reset (&key (n-inputs *n-inputs) (n-outputs *n-outputs)(vigilance *vigilance) (recalc-F3L1xy-postreset-p *recalc-F3L1xy-postreset-p))  ;;was, replace in let* &aux (res 0.0) (norm1 0.0) (norm2 0.0)  (n1 (+ (L2NORM p n-inputs) e)) n2 temp) uses &aux instead of let??
...
(let*
    ((res 0.0)
     ...
     (l2norm-yi-2-2 (L2NORM 'yi-2-2 *n-inputs)) ;;(l2norm-p (L2NORM 'p n-inputs))  ;;eg p=(P0 P1 P2 P3 P4)
     (l2norm-yi-2-2-e (+ l2norm-yi-2-2 e)) ;; (n1 (+  l2norm-p  e)) ;;e= .04?
     (n2)
     (temp)
     (l2norm-r)
  ...
  (setf  *art-data-text (append *art-data-text (list (format nil  "~%>>>>>> IN TEST-RESET >>>>~%STEP 1: FIND  RI,  vector components betw yi-2-1 and yi-2-2~% Step 5.1 Find max1yI-2-1-val  (of all yI-2-3)  and max2yi-2-2-val (of all yI-2-2)~%  max1yI-2-1-val= ~A  max2yI-2-2-val= ~A~%"   max1yI-2-1-val max2yi-2-2-val))))
  ...
  (setf  *art-data-text (append *art-data-text (list (format nil  "~%Step 1.2: FIND  ri-2-2-vals= ~A~%FOR each i;  ri-2-2= (- (/  yi-2-1  max1yi-2-1-val)  (/ yi-2-2  max2yi-2-2-val)) ;   yi-2-1  max1yi-2-1-val= ~A,  yi-2-2  max2yi-2-2-val= ~A~% WHERE yi-2-1-vals= ~A~%         and yi-2-2-vals= ~A"  ri-2-2-vals  max1yi-2-1  max2yi-2-2 yi-2-1-vals yi-2-2-vals ))))

   ;;STEP 2:  TEST FOR ADEQUATE INPUT to test for reset and not *skip-reset
  (cond
   ((and (> l2norm-yi-2-2-e *min-input-criteria)  ;; (> n1 *min-input-criteria) ;;was  0.2)
         (not *skip-reset))
    ...
    (when
      ...
      ;;Step 3.1 Find the largest F3? Y OUTPUT VALUE AT L1 OR L3?
      (cond
       (*find-F3L3y-max-output-p
        (multiple-value-setq (F3y-max-Index F3y-max-value)
            (find-largest-node 'yi-3-3 *n-outputs) ))          ;; (findLargestOutput)
       (*find-F3L1y-max-output-p
        (multiple-value-setq (F3y-max-Index F3y-max-value)
     ...
     (setf *art-data-text (append *art-data-text (list (format nil "==>>RESET: MAX F3 X and Ys in  LAYER 2 X and Y nodes. *reset-all-F3Layer-maxs-p= ~A" *reset-all-F3Layer-maxs-p))))
      ...
      ;;end when, l2norm-yi-2-2-e > clause    
      ))
   (T
